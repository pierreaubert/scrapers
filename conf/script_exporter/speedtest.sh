# https://github.com/billimek/prometheus-speedtest-exporter/blob/master/speedtest-exporter.sh
#!/bin/bash
...
n            printMetric "speedtest_download_bytes" "Download Speed" "gauge" "$download"
        ...
        done < <(/snap/bin/speedtest --accept-license --accept-gdpr -f tsv)
else
    ...
    do
        ...
        done < <(/snap/bin/speedtest --accept-license --accept-gdpr -f tsv --server-id $server_id)
    done
fi